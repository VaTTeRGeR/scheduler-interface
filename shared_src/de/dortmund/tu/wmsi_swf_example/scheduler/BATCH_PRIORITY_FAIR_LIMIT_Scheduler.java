...
public class BATCH_PRIORITY_FAIR_LIMIT_Scheduler implements Scheduler {
	...
	private long t_threshold_batch;
	private long t_threshold_priority = 4 * 60 * 60;
	...
	public void enqueueJob(Job job) {
		...
		final boolean isUserInBatch = t_now - t_last_submit < t_threshold_batch;
		final boolean isJobPriority = job.get(Job.TIME_REQUESTED) < t_threshold_priority;

		long jobPriority;
		
		if(isUserInBatch && isJobPriority) {
			if(isUserInQueue) {
